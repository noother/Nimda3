...
class LockdownPlugin extends Plugin {
		...
		foreach(Common::getBot()->servers as $Server) {
			foreach($Server->channels as $Channel) {
				...
				foreach($Channel->users as $User) {
					if(empty($User->modes[$Channel->id]) && $this->findPlugin('seen')->getVar($User->id) !== null) {
						$users[] = $User;
					}
				}
	...
	private function stopChannelLockdown() {
		...
		$usermodes = array();
		foreach($this->Channel->getVar('voiced_during_lockdown', []) as $nick) {
	...
	function onJoin() {
		...
		if($this->findPlugin('seen')->getVar($this->User->id) === null) {
	...
	function onMode() {
		...
		if($this->findPlugin('seen')->getVar($this->data['Victim']->id) === null) {
	...
	function onNick() {
		...
		foreach($this->Server->channels as $Channel) {
			if($Channel->getVar('lockdown_active')) {
				$voiced = $Channel->getVar('voiced_during_lockdown', []);
	...
	function onKick() {
		...
		$voiced = $this->Channel->getVar('voiced_during_lockdown', []);
